...
description: Een handleiding voor bijdragers over het schrijven van documentatie, het configureren van zijbalken en het beheren van de oorspronkelijke Docusaurus-versies.
---

# Documentatie- en versiehandleiding

Deze handleiding is bedoeld voor bijdragers en beheerders. Het behandelt hoe u documentatiepagina's toevoegt, de zijbalk configureert, de site lokaal uitvoert en versiebeheer beheert.
...
Alle documentatiebestanden bevinden zich in de map `website/docs/`:
- `website/docs/user/`: gebruikershandleidingen, installatie-instructies en probleemoplossing.
- `website/docs/technical/`: Architectuur, moduleontwerpen, repositorydetails en bouw-/releaseprocessen.
- `website/docs/contributing/`: Richtlijnen, standaarden en workflowinstructies voor bijdragers.

### Een nieuwe pagina maken

1. Maak een nieuw `.md` (of `.mdx`) bestand onder de juiste submap van `website/docs/`.
2. Voeg bovenaan het bestand de benodigde frontmatter toe:
...
Docusaurus ondersteunt standaard Markdown, MDX en verschillende ingebouwde interactieve functies:
...
Zeemeermindiagrammen worden volledig ondersteund in dit project. U kunt inline stroomdiagrammen of sequentiediagrammen schrijven:

```markdown
```mermaid
grafiek TD;
A[Markdown-bestand schrijven] --> B[sidebars.js configureren];
...
## 2. De zijbalk configureren

De zijbalknavigatiestructuur is gedefinieerd in `website/sidebars.js`.

### Een pagina toevoegen aan de zijbalk

1. Open `website/sidebars.js`.
2. Zoek de categorie waarin u het nieuwe document wilt laten verschijnen.
...
Dit project maakt gebruik van Docusaurus eigen documentatieversiebeheer om eerdere releases van de documentatie te archiveren.

- **Volgende (niet-uitgebrachte)** versie: komt overeen met bestanden in de actieve ontwikkelingstak. Bronbestanden worden rechtstreeks in `website/docs/` bewerkt.
...
Door het versiecommando uit te voeren, worden de volgende structuren gemaakt:
...
- **De komende release bijwerken (volgende)**: bewerk bestanden in `website/docs/`.
- **Een reeds uitgebrachte versie bijwerken (bijvoorbeeld 1.2.9)**: Bewerk het overeenkomstige bestand in `website/versioned_docs/version-1.2.9/`.
...
Voer deze opdrachten uit in de map `website/`:
...
Voordat u een commit uitvoert, voert u een productiebuild uit om te controleren op verbroken koppelingen en configuratiefouten:

```bash
npm run build
```
Eventuele verbroken interne links of ongeldige configuraties zorgen ervoor dat de build mislukt, waardoor implementatieproblemen worden voorkomen.
